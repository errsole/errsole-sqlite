...
class ErrsoleSQLite extends EventEmitter {
  ...
  async createTables () {
    const queries = [
      ...
      'CREATE INDEX IF NOT EXISTS idx_errsole_logs_v2_source_level_id ON errsole_logs_v2 (source, level, id)',
      'CREATE INDEX IF NOT EXISTS idx_errsole_logs_v2_source_level_timestamp ON errsole_logs_v2 (source, level, timestamp)',
      'CREATE INDEX IF NOT EXISTS idx_errsole_logs_v2_hostname_pid_id ON errsole_logs_v2 (hostname, pid, id)',
      'CREATE INDEX IF NOT EXISTS idx_errsole_id ON errsole_logs_v2 (errsole_id);',
      'CREATE INDEX IF NOT EXISTS idx_errsole_notifications_hostname_hashed_message_created_at ON errsole_notifications (hostname, hashed_message, created_at)',
  ...
  async flushLogs () {
    ...
    const query = `INSERT INTO errsole_logs_v2 (timestamp, hostname, pid, source, level, message, meta, errsole_id) VALUES ${placeholders}`;
  ...
  async searchLogs (searchTerms, filters = {}) {
    ...
    const query = `SELECT id, hostname, pid, source, timestamp, level, message, errsole_id FROM errsole_logs_v2 ${whereClause} ORDER BY id ${sortOrder} ${limitClause}`;
    ...
    try {
      ...
      do {
        deletedRowCount = await new Promise((resolve, reject) => {
          this.db.run(
            'DELETE FROM errsole_logs_v2 WHERE id IN (SELECT id FROM errsole_logs_v2 WHERE timestamp < ? LIMIT 1000)',
            [expirationTime],
  ...
  async insertNotificationItem (notification = {}) {
    ...
    try {
      ...
      const previousNotificationItem = await new Promise((resolve, reject) => {
        db.get(fetchPreviousNotificationQuery, [hostname, hashedMessage], (err, row) => {
          if (err) return reject(err);
          resolve(row || null);
        });
      });
      ...
      `;
      await new Promise((resolve, reject) => {
        db.run(insertNotificationQuery, [hostname, errsoleId, hashedMessage], err => {
          if (err) return reject(err);
          resolve();
        });
      });
      ...
      const todayNotificationCount = await new Promise((resolve, reject) => {
        db.get(countTodayNotificationsQuery, [hashedMessage], (err, result) => {
          if (err) return reject(err);
    ...
    try {
      ...
      do {
        ...
        await new Promise((resolve, reject) => {
          ...
          this.db.run(
          `DELETE FROM errsole_notifications WHERE id IN (${placeholders})`,
          idsToDelete,
          function (err) {
            if (err) return reject(err);
            resolve(this.changes);
