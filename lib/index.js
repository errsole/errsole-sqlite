...
class ErrsoleSQLite extends EventEmitter {
  ...
  async createTables () {
    this.createLogsTableQuery = `CREATE TABLE IF NOT EXISTS ${this.logsTable} (
  ...
  async ensureLogsTTL () {
    const DEFAULT_LOGS_TTL = 7 * 24 * 60 * 60 * 1000; // 30 days in milliseconds
    ...
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        this.db.run('BEGIN TRANSACTION;', err => {
          if (err) return reject(err);
          this.db.run(`DROP TABLE IF EXISTS ${this.logsTable};`, err => {
            if (err) return reject(err);
            this.db.run(this.createLogsTableQuery, err => {
              if (err) return reject(err);
